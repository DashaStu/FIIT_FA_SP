            ...
            if (inner_it != it->second.end()) {
                ...
                if (!path.empty()) {
                    ...
                    if (stream) {
                        ...
                        stream.flush(); // Принудительный сброс буфера
                    }
                }

                // Вывод в консоль (добавлена проверка на UTF-8 и длинные строки)
                if (console) {
                    // Обрезаем слишком длинные сообщения для консоли
                    std::string console_msg = message;
                    if (console_msg.length() > 200) {
                        console_msg = console_msg.substr(0, 197) + "...";
                    }
                    std::cout << console_msg << std::endl;
                    std::cout.flush(); // Гарантируем вывод
                }
            }
        }
